...
As part of the project verification, we captured the traffic generated by the application using Wireshark.

**Capture Setup:**

* **Interface:** Loopback Adapter (localhost traffic).
* **Filter:** `tcp.port == 5555`.

**Observations:**

1. **3-Way Handshake:** Successfully identified the `SYN`, `SYN-ACK`, `ACK` sequence establishing the connection.
2. **Data Transmission:** Chat messages were observed in `PSH, ACK` packets. The payload clearly showed our custom protocol (e.g., `MSG|User2|Hello`).
3. **Keep-Alive / ACK:** TCP `ACK` packets were seen confirming the receipt of messages, ensuring reliability.
4. **Connection Termination:** Observed `FIN, ACK` packets when closing the client application.


**Course:** Computer Networks | **Project Phase:** 2 (Final)